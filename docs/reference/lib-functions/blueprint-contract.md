...
The exception is a backfill whose libs all returned 0 rows with `empty_result: "no_change"`: it keeps the existing rows instead of rebuilding, so audits do not execute and constraint violations are not reported (2), and a schema change other than an added column is deferred with the rebuild (6). See [Rebuild from an empty fetch](/reference/pipeline/run-types/#rebuild-from-an-empty-fetch).
