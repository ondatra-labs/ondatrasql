---
description: Reference for OndatraSQL lib functions. API dict configuration, fetch and push contracts, blueprint stability rules, and Starlark module APIs.
draft: false
title: Lib Functions
weight: 10
---
Reference for lib function configuration, contracts, and runtime modules. For why lib functions exist and how they fit the architecture, see [Concepts: Lib Functions](/concepts/lib-functions/).

- [API Dict](/reference/lib-functions/api-dict/) — configuration: auth, endpoints, rate limits, pagination
- [Fetch Contract](/reference/lib-functions/fetch-contract/) — inbound: kwargs, return format, pagination, async
- [Push Contract](/reference/lib-functions/push-contract/) — outbound: change types, batch modes, delivery guarantees
- [Blueprint Contract](/reference/lib-functions/blueprint-contract/) — stability guarantees, dynamic behavior, empty-run handling
- [Starlark Modules](/reference/lib-functions/starlark-modules/) — http, env, json, csv, xml, time, builtins
