...
func syncSink(batchMode string) *libregistry.LibFunc {
	return &libregistry.LibFunc{
		...
		IsSink:   true,
		FuncName: "push",
		PushConfig: &libregistry.PushConfig{
			BatchSize:     100,
			BatchMode:     batchMode,
			MaxConcurrent: 1,
		},
...
func sinkWithRateLimit(per string) *libregistry.LibFunc {
	s := syncSink("sync")
	s.PushConfig.RateLimit = &libregistry.RateLimitConfig{Requests: 10, Per: per}
	return s
}

func sinkWithPoll(interval, timeout string) *libregistry.LibFunc {
	s := syncSink("async")
	s.PushConfig.PollInterval = interval
	s.PushConfig.PollTimeout = timeout
	return s
}

func sinkWithConcurrency(mode string, concurrent int) *libregistry.LibFunc {
	s := syncSink(mode)
	s.PushConfig.MaxConcurrent = concurrent
	return s
}
...
func TestValidate_TableAsyncAllowed(t *testing.T) {
	...
	models := []*parser.Model{{Target: "sync.contacts", Kind: "table", Push: "async_push"}}

	if err := ValidateModelPushCompat(models, reg); err != nil {
...
func TestValidate_MergeWithoutUniqueKey(t *testing.T) {
	...
	models := []*parser.Model{{Target: "sync.contacts", Kind: "merge", Push: "push"}}

	err := ValidateModelPushCompat(models, reg)
	if err == nil {
...
func TestValidate_TrackedWithoutGroupKey(t *testing.T) {
	t.Parallel()
	reg := makeReg(map[string]*libregistry.LibFunc{"push": syncSink("sync")})
	models := []*parser.Model{{Target: "sync.invoices", Kind: "tracked", Push: "push"}}
...
func TestValidate_NilRegistryWithPushModel(t *testing.T) {
	...
	err := ValidateModelPushCompat(models, nil)
	if err == nil {
...
func TestValidate_ValidConfig(t *testing.T) {
	t.Parallel()
	reg := makeReg(map[string]*libregistry.LibFunc{"push": syncSink("sync")})
	models := []*parser.Model{{Target: "sync.contacts", Kind: "merge", Push: "push", UniqueKey: "id"}}
...
func TestValidate_NoPushModels(t *testing.T) {
	...
	if err := ValidateModelPushCompat(models, nil); err != nil {
...
func TestValidate_TableMaxConcurrentAllowed(t *testing.T) {
	...
	models := []*parser.Model{{Target: "sync.contacts", Kind: "table", Push: "push"}}
...
// Docs claim: "All kinds except scd2 support @sink."
