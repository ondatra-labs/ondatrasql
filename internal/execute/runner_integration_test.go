...
func TestRun_Extension_FromRepo(t *testing.T) {
	...
	// Skip if download fails (community repo may not have builds for this DuckDB version).
	p.AddModel("staging/ext_repo.sql", `-- @kind: table
-- @extension: inet FROM community
SELECT 1 AS id, 'test' AS val
`)
	modelPath := filepath.Join(p.Dir, "models", "staging/ext_repo.sql")
	model, err := parser.ParseModel(modelPath, p.Dir)
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	...
	result, err := runner.Run(context.Background(), model)
	if err != nil {
		if strings.Contains(err.Error(), "HTTP") || strings.Contains(err.Error(), "download") {
			t.Skipf("skipping: community extension not available: %v", err)
		}
		t.Fatalf("run: %v", err)
	}
	if result.RowsAffected != 1 {
