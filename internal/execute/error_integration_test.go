...
// --- Starlark script error paths (covers script.go) ---

func TestRun_Script_SyntaxError(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
	p := testutil.NewProject(t)
	p.AddModel("staging/bad_script.star", `this is not valid starlark!!!`)

	_, err := runModelErr(t, p, "staging/bad_script.star")
	if err == nil {
		t.Fatal("expected error from invalid Starlark syntax")
	}
}

func TestRun_Script_RuntimeError(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
	p := testutil.NewProject(t)
	p.AddModel("staging/fail_script.star", `fail("intentional error")`)

	_, err := runModelErr(t, p, "staging/fail_script.star")
	if err == nil {
		t.Fatal("expected error from fail()")
	}
}

func TestRun_Script_EmptyData(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
	p := testutil.NewProject(t)
	// Script that runs but collects no data
	p.AddModel("staging/empty_script.star", `
# Do nothing - no save() call
x = 1 + 1
`)
	result, err := runModelErr(t, p, "staging/empty_script.star")
	if err != nil {
	...
	if result != nil {
		for _, w := range result.Warnings {
			if strings.Contains(w, "no data") {
				t.Logf("got expected warning: %s", w)
			}
		}
	}
}

func TestRun_Script_WithConstraintFailure(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
	p := testutil.NewProject(t)
...
`)
	...
	if err != nil {
		t.Logf("constraint error (expected): %v", err)
	}
}
...
// --- Starlark script with save.row that materialize succeeds ---

func TestRun_Script_SuccessfulMaterialize(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
	p := testutil.NewProject(t)
	p.AddModel("staging/good_script.star", `# @kind: table
save.row({"id": 1, "name": "Alice"})
`)
	result := runModel(t, p, "staging/good_script.star")
	if len(result.Errors) > 0 {
...
// --- Starlark script with constraint failure (covers script.go:196-198) ---

func TestRun_Script_ConstraintFailure(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
	p := testutil.NewProject(t)
...
// --- Starlark script with audit failure (covers script.go:245-247, 263-265) ---

func TestRun_Script_AuditFailure(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
	p := testutil.NewProject(t)
...
func TestRun_Script_MaterializeError(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
	p := testutil.NewProject(t)
	// Script that saves data but with an invalid kind that will cause materialize to fail
	model := &parser.Model{
	...
	_, err := runner.Run(context.Background(), model)
	if err == nil {
		t.Fatal("expected materialize error for unknown kind")
	}
}

// --- Starlark script with schema evolution (covers script.go:167-171) ---

func TestRun_Script_SchemaEvolution(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping in short mode")
	}
	p := testutil.NewProject(t)
	...
	p.AddModel("staging/script_evo.star", `# @kind: append
save.row({"id": 1, "name": "Alice"})
`)
	result := runModel(t, p, "staging/script_evo.star")
	if len(result.Errors) > 0 {
		t.Fatalf("first run errors: %v", result.Errors)
	}

	// Second run with extra column — schema evolution
	testutil.WriteFile(t, p.Dir, "models/staging/script_evo.star", "# @kind: append\nsave.row({\"id\": 2, \"name\": \"Bob\", \"new_col\": 42})\n")
	result2 := runModel(t, p, "staging/script_evo.star")
	if len(result2.Errors) > 0 {
		t.Fatalf("second run errors: %v", result2.Errors)
	}
	t.Logf("script schema evolution: run_type=%s warnings=%v", result2.RunType, result2.Warnings)
}
